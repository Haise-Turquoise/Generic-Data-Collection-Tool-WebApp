...
export default class SpreadsheetApisService {
  ...
  */
  async sendAttributeAndCatagory(){
    ...
    let COATreeData: CategoryTreeDoc[] = await this.COATreeRepository.findAll();
    const AttributeData: AttributeDoc[] = await this.ColumnNameRepository.findAll();
    // JSON object that will contain all the CategoryTrees and Attributes
    let dataToSend  = {'Categories':[], 'Attributes':[]};

    const sheetNameList: ObjectId[] = [];
    ...
    const fullCategoryGroupList: CategoryGroupDoc[] = await this.COAGroupRepository.batchFind(categoryGroupList);
    const fullCategoryList: CategoryDoc[] = await this.COARepository.batchFindFull(categoryList)
    const fullSheetNamelist: SheetNameDoc[] = await this.sheetNameRepository.batchFind(sheetNameList);
  ...
  async findOrgWithMasterValueEntries(){
    return this.masterValueRepository.findAll().then((entries: MasterValueDoc[])=>{
      const organizations: MasterValueDoc["org"][] = [];
...
// Insert all the Attributes to the JSON Object
function pushAttributes(dataToSend: { Categories: any[], Attributes: any[] }, AttributeData: AttributeDoc[]){
...
// Some COATrees in the database are a child of another COA tree
// This function moves child COATrees into a childCategory array
function organizeCOATree(COATreeData: CategoryTreeDoc[]){
  for (let i = 0; i < COATreeData.length; i++){
    ...
    if (COATreeData[i].parentId){
      ...
      if (pos){
        ...
        //@ts-ignore unsure about this logic
...
function searchColumn(tree: (CategoryTreeDoc & {childCategories?: CategoryTreeDoc[]})[], parentId: ObjectId, firstIteration = true): CategoryTreeDoc[] | undefined {
  for (let i = 0; i < tree.length; i++){
    ...
    if (treeItem._id && treeItem._id.toString() === parentId.toString()){
      ...
      const childCategoryArray: CategoryTreeDoc[] | undefined = searchColumn(treeItem.childCategories, parentId);
...
async function pushCategory(
    dataToSend,
    COATreeData: CategoryTreeDoc[],
    fullCategoryGroupList: CategoryGroupDoc[],
    fullCategoryList: CategoryDoc[],
    fullSheetNamelist: SheetNameDoc[]
  ){
  for (let i = 0; i < COATreeData.length; i++){
    ...
    if (COATree._id){
      ...
      for (let item in COATree.categoryId){
        for (let secondItem in fullCategoryList){
          // TODO changed logic here test later
          if (fullCategoryList[secondItem].id === COATree.categoryId[item].toString()){
