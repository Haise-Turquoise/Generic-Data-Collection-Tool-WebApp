  ...
  if (err) {
    ...
    if(isProduction) {
      if (err.isKnown) {
        ...
        if (err.code === CONSTANTS.DUPLICATE_FIELD_ERROR) err = errorHandler.handleDuplicateFieldsDB(err);
        if (err.name === CONSTANTS.VALIDATION_ERROR) err = errorHandler.handleValidationErrorDB(err);
