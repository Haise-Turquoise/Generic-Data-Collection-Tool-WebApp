...
const AppSysController = (() => {
  ...
  return {
    ...
    fetchAppSys: async (_id: string): Promise<AppSys | null> =>
      AppSysAxios.post('/fetchAppSys', { _id }).then(res => res.data.AppSys),
    create: async (AppSys: AppSys): Promise<AppSys | null> => AppSysAxios.post('/create', { AppSys }).then(res => res.data.AppSys),
