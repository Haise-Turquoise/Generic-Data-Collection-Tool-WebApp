...
import { selectFactoryValueById } from '../../store/common/REST/selectors';
  ...
  const RequestManagementTable = () => {
    ...
    const approve_actions = { icon: ThumbUpIcon, tooltip: 'Approve Request', onClick: onClickApprove };
    ...
    useEffect(()=>{setRowNum(toBeApproved.length)}, [toBeApproved]);
  
    // @ts-ignore
    return <MaterialTable key={readRowNum} columns={columns} data={toBeApproved}  options={options} actions = {[approve_actions, reject_actions]}/>;
  };
