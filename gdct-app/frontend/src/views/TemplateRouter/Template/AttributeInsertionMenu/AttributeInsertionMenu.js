...
class attributeInsertionMenu extends React.Component {
  ...
  updateAttributes(attributeGroup) {
    const property_name = document.getElementById('AttributeGroup');
  ...
  insert() {
    const input_fields = document.getElementById('AttributeGroup');
    // @ts-ignore
    if (input_fields.selectedIndex) {
      // @ts-ignore
      const { text } = input_fields[input_fields.selectedIndex];
      // @ts-ignore
