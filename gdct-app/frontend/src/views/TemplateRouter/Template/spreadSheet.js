...
class SpreadSheet extends Component {
  ...
  insertVariance = varianceSelection => {
    ...
    const targetCol =
      findResult > 0 ? Number(findResult) : Number(findLastAttributeCol(currSheet) + 1);
  ...
  disablePreview() {
    ...
    this.insertedPreview.forEach(coord => {
      ...
      this.sheet.cellText(
        categoryMapping[currentSheetIndex][COAID],
        attributeMapping[currentSheetIndex][attributeId],
        '',
        currentSheetIndex,
      );
    });
  ...
  lineNumberInsertion(reRender = false) {
    ...
    let categoryIDs = Object.keys(categoryMap).sort(
      (id1, id2) => categoryMap[id1] - categoryMap[id2],
    );
  ...
  fileImportHandler(event) {
    excelImportHandler(event, data => {
      this.sheet.loadData(data).reRender();
    });
  }
      ...
      <div>
        <div style={{ display: 'flex' }}>
          ...
          <Button
            variant="outlined"
            color="primary"
            onClick={() => this.downloadTemplate(this.sheet.getData())}
          >
            Download Template
          </Button>
          <input type="file" accept=".xlsx, .xlsm" onChange={e => this.fileImportHandler(e)} />
        </div>
