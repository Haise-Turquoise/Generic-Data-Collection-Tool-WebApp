...
export const approvePermission = (rowData, applierUser, user, resolve, reject) => dispatch => {
  ...
  userController.updateToBeApproved(userCopy).then(result=>{if(resolve){resolve()}})
  userController.updatePendingPermissions(applierUserCopy).then(result=>{if(resolve){resolve()}})
